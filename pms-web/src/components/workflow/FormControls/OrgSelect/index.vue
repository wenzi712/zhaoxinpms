<template>
    <div class="org-select" :class="{ 'org-select__button': buttonType === 'button' }">
        <div class="tags">
            <el-button v-if="buttonType === 'button'" icon="el-icon-plus" style="margin-bottom: 6px" @click="show = true">
                {{ title }}
            </el-button>
            <div class="input-box" :class="{ 'as-input': buttonType === 'input' }" @click="show = true">
                <el-tag v-for="(item, i) in selectedData" :key="item.key" v-bind="tagConfig" class="org-tag" @close="onClose(item, i)">
                    {{ item.name }}
                </el-tag>
            </div>
        </div>
        <org-transfer ref="transfer" :value="innerValue" :title="title" :visible.sync="show" :type="type" @confirm="onConfirm" />
    </div>
</template>
<script>
