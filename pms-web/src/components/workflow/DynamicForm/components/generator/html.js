...
function buildFormTemplate(conf, child, type) {
    ...
    let str = `<el-form ref="${conf.formRef}" :model="formData" :rules="ruleList" size="${conf.size}" ${disabled} label-width="${
        conf.labelWidth
    }px" ${labelPosition}>
      ${child}
...
const layouts = {
    colFormItem(element, noFormItem = false) {
        ...
        let rules = '';
        let label = `label="${noFormItem ? '' : element.label}"`;
        let prop = `prop="${element.vModel}"`;
        if (noFormItem) {
...
const tags = {
    ...
    'el-rate': el => {
        const { disabled, vModel } = attrBuilder(el);
        // 由于max变量未被使用，我们直接在返回语句中使用它
        const allowHalf = el['allow-half'] ? 'allow-half' : '';
        const showText = el['show-text'] ? 'show-text' : '';
        const showScore = el['show-score'] ? 'show-score' : '';
        const maxAttr = el.max ? `:max='${el.max}'` : '';
        return addPropToTag(tag, `${disabled} ${maxAttr} ${allowHalf} ${showText} ${showScore}`);
    },
    'el-color-picker': el => {
        const { disabled, vModel } = attrBuilder(el);
        const size = `size="${el.size}"`;
